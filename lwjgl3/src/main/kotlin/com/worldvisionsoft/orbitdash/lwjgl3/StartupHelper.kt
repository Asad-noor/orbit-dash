...
// Kotlin port of the gdx-liftoff StartupHelper.
 ...
 * `-XstartOnFirstThread` argument, which is required on macOS for LWJGL3
 * to function. Also helps on Windows when users have names with characters
 * from outside the Latin alphabet, a common cause of startup crashes.
 *
...
object StartupHelper {
     ...
     * non-Latin-alphabet characters (without this, most LWJGL3 apps fail
     ...
     * old JVM, so it can be accessed in the same place; keeps the old JVM running if enabled
    ...
    fun startNewJvmIfRequired(redirectOutput: Boolean = true): Boolean {
        ...
        if (!osName.contains("mac")) {
            if (osName.contains("windows")) {
                ...
                System.setProperty(
                    "user.name",
                    "User_${prevUser.hashCode()}_GDX${com.badlogic.gdx.Version.VERSION}"
                )
